...
class Closed_form_estimator(RegressorMixin):
    ...
    def fit(self, X, y, evalset = None, **kwargs):
        ...
        permuted_y = self._permute(y)
        if not (self.is_fitted_ and self.warm_start):
            self._create_params(X, y, permuted_y)
            ...
            self.old_loss = 1e16
            self.start_iter = 0
        self._run_optimization(X, y, permuted_y, evalset)
    ...
    def _run_optimization(self, X, y, permuted_y, evalset):
        ...
        constants = self._init_constants(X, y, permuted_y)
        X_true, y_true, permuted_y_true = (constants["X"], constants["y"], constants["permuted_y"])
        intercept_loss = torch.sqrt(((y_true - y_true.mean())**2).sum())
        for e in range(self.start_iter, self.max_iter + self.start_iter): 
            y_pred, permuted_y_pred = self._forward(constants)
            true_loss, false_loss = self._BKK_criterion( y_true, permuted_y_true, y_pred, permuted_y_pred)
            loss = true_loss + torch.abs(false_loss - intercept_loss)
            loss.backward()
            self._optim.step()
    ...
    def _forward(self, constants):
        if self.elastic_feature_sparsity:
            ...
            y_hat, permuted_y_hat = self._inversion_forward( constants, feature_sparsity = False)
            sparse_y_hat, sparse_permuted_y_hat = self._inversion_forward( constants, 
                                                                   feature_sparsity = True)
            return (y_hat * mu + (1 - mu) * sparse_y_hat, 
                    permuted_y_hat * mu + (1 - mu) * sparse_permuted_y_hat)
    ...
    def _permute(self, y):
        return np.concatenate([shuffle(y, random_state = self.random_state + seed_update).reshape(-1,1) for         seed_update in range(self.n_permut)], axis = 1)
    
    def _BKK_criterion(self,  y_true, permuted_y_true, y_pred, permuted_y_pred):
        return torch.norm(y_true - y_pred), self._row_wise_norm(permuted_y_true, permuted_y_pred)
    
    def _row_wise_norm(self, permuted_y_true, permuted_y_pred):
        row_norm = torch.sqrt(((permuted_y_true - permuted_y_pred)**2).sum(dim= 0))
    ...
    def _create_params(self, X, y, my):
    ...
    def _init_constants(self, X, y, permuted_y):
        if self.eigen_decomposition:
            XTX = X.T.dot(X)
            u, eigen, uT = np.linalg.svd(XTX)
            Xu = X.dot(u)
            utXty = Xu.T.dot(y)
            utXtmy = Xu.T.dot(permuted_y)
            constants = {"X" : X,
                         "y":y, 
                         "permuted_y" : permuted_y, 
                         "eigen":eigen, 
                         "u" : u, 
                         "Xu": Xu,
                         "utXty" : utXty, 
    ...
    def _inversion_forward(self, constants, feature_sparsity):
        ...
        if feature_sparsity:
            ...
            sparse_XTmy = sparse_vector @ XTmy
            ...
            permuted_y_hat = projection_matrix @ sparse_XTmy
            ...
            permuted_y_hat = projection_matrix @ XTmy
            return y_hat, permuted_y_hat
    
    def _eigen_decomposition_forward(self, constants):
        X, y, permuted_y, eigen, u, Xu, utXty, utXtmy = constants.values()
        projection_matrix = Xu * self._get_diag(eigen)
        y_hat = projection_matrix @ utXty
        permuted_y_hat = projection_matrix @ utXtmy
        return y_hat, permuted_y_hat
